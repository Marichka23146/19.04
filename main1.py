...
class Camera():
    ...
    def apply(self,traget):
        return traget.rect.move(self.state.topleft)
    def update(self, traget):
        self.state = self.camera_func(self.state, traget.rect)
        
    def camera_config(camera,traget_rect):
            l,t,_,_= traget_rect 
            _,_,w,h = camera
            l,t = -1 + W/2, t + H/2
            l = min(0,1)    
            t = min(0,t)
            l = max(-(camea.wicth -W),l)
